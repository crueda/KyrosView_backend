...
LOG_FILE = config['directory_logs'] + "/kyrosView-lastMonth.log"
LOG_FOR_ROTATE = 10
...
def openJsonFiles():
	...
	for k in users.keys():
		userJsonFile[k] = open(JSON_DIR + '/users/lastMonth/' + str(k) + '.json', "a+")
...
os.system("rm -f " + JSON_DIR + "/users/lastMonth/*.json")
openJsonFiles()
