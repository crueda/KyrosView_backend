...
def openJsonFiles():
	...
	for k in devices.keys():
		activityJsonFile[k] = open(JSON_DIR + '/devices/activity/last15days/' + str(k) + '.json', "a+")
...
def getTracking(deviceId):
	...
	msegLast = str((int(time.time())*1000)- 1296000000)
...
os.system("rm -f " + JSON_DIR + "/devices/activity/last15days/*.json")
openJsonFiles()
