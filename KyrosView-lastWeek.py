...
LOG_FILE = config['directory_logs'] + "/kyrosView-lastWeek.log"
LOG_FOR_ROTATE = 10
...
def openJsonFiles():
	...
	for k in users.keys():
		userJsonFile[k] = open(JSON_DIR + '/users/lastWeek/' + str(k) + '.json', "a+")
...
os.system("rm -f " + JSON_DIR + "/users/lastWeek/*.json")
openJsonFiles()
